...
export const loginService = async (email: string, password: string) => {
   ...
   if (user.password)
      if (!comparePassword(password, user.password)) throw { code: 500, msg: 'Wrong password' }
...
export const registerService = async (email: string, password: string, name: string) => {
   ...
   const createdUser = await UserController.createUserController({ email, password: hashedPassword, name })
